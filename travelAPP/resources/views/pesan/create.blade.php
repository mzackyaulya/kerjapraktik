    ...
    <div class="card-body">
        <form action="{{ route('pesan.store', $selectedJadwal ? $selectedJadwal->id : '') }}" method="POST">
                ...
                <select name="jadwal_id" id="jadwal_id" class="form-control" required>
                    ...
                    @foreach($jadwal as $item)
                        <option value="{{ $item->id }}"
                            {{ isset($selectedJadwal) && $selectedJadwal->id == $item->id ? 'selected' : '' }}>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="seet" class="form-control" value="{{ old('seet') }}" required>
                @error('seet')
                    <small class="text-danger">{{ $message }}</small>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="number" name="jumlah_orang" class="form-control" value="{{ old('jumlah_orang') }}" min="1" required>
                @error('jumlah_orang')
                    <small class="text-danger">{{ $message }}</small>
                @enderror
            </div>
            ...
            <!-- Tombol Submit -->
            <button type="submit" class="btn btn-primary col-12">Simpan Pemesanan</button>
